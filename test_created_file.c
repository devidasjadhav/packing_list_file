...
int list_to_file( uint64_t fd, struct node_s * head){
    struct emp_s *temp, temp_unpack;
    uint8_t writen_bytes ,buffer[60];
    for(struct node_s * node = head; node != NULL ; node = node->next){
       	temp = CONTAINER_OF(node,struct emp_s ,node); 
...
struct node_s * list_from_file( uint64_t fd , struct node_s **head){
	...
	while(ret > 0){
		...
		if(ret == 0)
			break;
		unpacker_emp(&temp_unpack,buffer);
		create_node(temp_unpack.eid,temp_unpack.name,temp_unpack.exp,head);
...
int read_from_file( uint64_t fd){
	struct emp_s temp_unpack;
	uint8_t ret = 1,number = 0, buffer[25];
	while(ret > 0){
		number ++;
...
/*
void list_from_file( uint64_t fd){
	struct emp_s temp_pack;
	uint8_t ret = 1,number = 0, buffer[60];
...
while ((nread = getline(&line, &len, stream)) != -1) {
    ...
    if(len == 0)
            break;
    temp_pack.eid = count;
    temp_pack.exp = strlen(line);
    strcpy(temp_pack.name,line);
    packer_emp(temp_pack,buffer);
...
/*    head = NULL;
    ...
    iterate_list(head);
  */  
    fd = open("./dummy", O_RDONLY | O_CREAT);
